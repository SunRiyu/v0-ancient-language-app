 ...
 * useSearchParamsを使用するため、Suspense配下で呼び出す必要があります。
 */
function MindmapContent() {
  const router = useRouter()
  const searchParams = useSearchParams()
  // URLから 'name' パラメータを取得。取得できない場合は '未知の語源' をデフォルトにする
                    ...
                    >
                      ...
                      <div className="relative w-16 h-16 sm:w-20 sm:h-20 rounded-lg bg-gradient-to-br from-[#3d3733] to-[#2a2520] border-2 border-[#5a5450] shadow-lg flex items-center justify-center overflow-hidden group hover:border-[#afd2e1] transition-all duration-300">
                        <div className="flex flex-col items-center gap-1">
                          <svg
                            className="w-6 h-6 sm:w-8 sm:h-8 text-[#afd2e1]"
                            viewBox="0 0 24 24"
                            fill="none"
                            stroke="currentColor"
                            strokeWidth="2"
...
/**
 * ページのエントリーポイント
 * ビルド時の Prerender Error を防ぐために Suspense でラップします。
 */
export default function EtymologyMindmapPage() {
  return (
    <Suspense fallback={
      <div className="min-h-screen bg-[#2a2520] flex items-center justify-center text-[#f5f5f1]">
        読み込み中...
      </div>
    }>
      <MindmapContent />
